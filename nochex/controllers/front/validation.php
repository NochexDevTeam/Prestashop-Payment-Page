...
class NochexApcValidationModuleFrontController extends ModuleFrontController
{
    ...
    public function postProcess()
    {
		...
		$id_order = Order::getOrderByCartId(Tools::getValue('order_id'));
            ...
            if (Tools::getValue('optional_2') == "callback") {
				...
				$nochexapc->validateOrder((int)Tools::getValue("order_id"), Configuration::get('PS_OS_PAYMENT'), Tools::getValue("amount"), $nochexapc->displayName, $responses, $extras, null, false, $custom);
				Tools::redirectLink(__PS_BASE_URI__.'order-confirmation.php?key='.$custom.'&id_cart='.(int)Tools::getValue("order_id").'&id_module='.(int)$nochexapc->id.'&id_order='.(int)$id_order);
